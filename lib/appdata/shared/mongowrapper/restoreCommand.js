 ...
 * @returns {*}
 */
Restore.prototype.run = function() {

  if (!this.paramsMap.path) {
    throw new Error('Path to the import file (or containing directory) is mandatory');
  }

  var params = buildParamsAry(this.paramsMap);

  return child_process.spawn(RESTORE, params);
};
