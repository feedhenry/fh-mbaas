...
function getMongoHost(hostList, port,role, cb) {
  ...
  async.everyLimit(hosts, 1, function(host, callback) {
    _getMongoHost(host, port, role, function(err, master) {
      if (err) {
        // master not found. Try next host.
        return callback(!err);
      } else {
        // master found. Send back the master.
        cb(null, master);
        // interrupt the loop
        callback(false);
      }
    ...
    if (notFound) {
      return cb('Unable to detect master instance');
    }
    // If it has been found, cb has already been called.
  });
}
