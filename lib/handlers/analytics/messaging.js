...
module.exports = function (messagingClient) {
  return {
    "createAppMessage": function appMessage(req, res, next) {
      ...
      // Inject mbaasid into each message before sending on to messaging
      _.each(messages, addMBaasIDToMessage);
      logger.debug('MBaaS ID ['+fhconfig.value('fhmbaas.mbaasid')+'] added to messages', messages);

      messagingClient.createAppMessage(topic, validateAppMessages(sender,messages), function done(err) {
        if (err) logger.warn("app messaging: error occurred sending message to messaging ",err);
      }); 
    }
  };
};
...
function addMBaasIDToMessage(message){
  var mbaasid = fhconfig.value('fhmbaas.mbaasid');
  message.mbaasid = mbaasid || 'missing mbaasid';
}