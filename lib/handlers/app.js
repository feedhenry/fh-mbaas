...
messagingConfig.debug = true;
var messagingClient = require('../clients/messaging/index')(messagingConfig);
...
router.use("/:domain/:environment/:projectid/:appid",fhmbaasMiddleware.auth.app);
router.use('/:domain/:environment/:projectid/:appid/appforms', require('./app/forms.js'));
router.post("/:domain/:environment/:projectid/:appid/events", fhmbaasMiddleware.events.create,end);
router.post("/:domain/:environment/:appid/message/:topic", fhmbaasMiddleware.auth.app, appMessageHandler(messagingClient).createAppMessage);
