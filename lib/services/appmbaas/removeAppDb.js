'use strict';
//this function is resposible for removing the data and db associated with an app in a given environment.
...
module.exports = function removeAppDb(mongo, domain, appModel, environment, next) {
  if (!appModel.migrated && appModel.type !== 'openshift3') {
    ...
    mongo.dropDb(config, appModel.dbConf.user, appModel.dbConf.name, function complete(err) {
      if (err) {
