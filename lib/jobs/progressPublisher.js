 ...
 * @constructor
 */
function ProgressPublisher(emitter, publisher, queueSize) {
  ...
  self.evtEmitter.on(STATUS_EVENT, function(status){
    ...
    self.publishMessages(self.messageQueue, publisher);
  });

  self.evtEmitter.on(PROGRESS_EVENT, function(progress){
    self.messageQueue.push({type: PROGRESS_EVENT, data: progress});
    self.publishMessages(self.messageQueue, publisher);
  });
  ...
  self.evtEmitter.on(FINISH_EVENT, function(message){
    ...
    self.publishMessages(self.messageQueue, publisher, true);
  });
