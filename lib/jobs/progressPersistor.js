...
var _logger = fhConfig.getLogger();
...
function publish(model, message, cb) {
  ...
  switch (message.type) {
    ...
    case FINISH_EVENT:
      logger.debug('Persisting event', message);
      persistStatusChange.call(this, model, 'complete', cb);
      return;
    case FAIL_EVENT:
      logger.debug('Persisting event', message);
      persistStatusChange.call(this, model, 'failed', cb);
      return;
    default:
      return;
  }
...
ProgressPersistor.prototype.listen = function (emitter, model, cb) {
  ...
  this.progressPublisher = new ProgressPublisher(emitter, this.publisherFunction, 1, self.logger);
