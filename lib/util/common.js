...
function formatDbUri(dbConf, replicaSetName) {
  var dbUrl = util.format('mongodb://%s:%s@%s:%s/%s', dbConf.user, dbConf.pass, dbConf.host, dbConf.port, dbConf.name);
  if (replicaSetName && replicaSetName.length > 0) {
    dbUrl = dbUrl + "?replicaSet=" + replicaSetName;
  }
  return dbUrl;
}
