...
// Sends a request to fh-ditch to check if any migration jobs are active for the given app
function checkMigrationJobs(appName, cb) {
  const url = getDitchUrl('/admin/checkmigration');
  request.post({
    url: url,
    headers: {
      'x-fh-service-key': fhconfig.value('fhditch.service_key')
    ...
    if (err) {
      logger.error({error: err, body: body}, 'Error checking migration jobs for');
      return cb(err);
    } else if (response && response.statusCode !== 200) {
      const message = `Error checking migration jobs, fh-ditch returned status ${response.statusCode}`;
...
exports.checkMigrationJobs = checkMigrationJobs;