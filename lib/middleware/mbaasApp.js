...
var fhconfig = require('fh-config').getConfig();
...
function createDbMiddleware(req, res, next){
  ...
  createDb({fhconfig: fhconfig, model: model}, function(err, result){
...
function removeDbMiddleware(req, res, next) {
  var model = req.appMbaasModel;
  removeDb({fhconfig: fhconfig, model: model}, function(err, result){
    return next(err, result);
  });
}
...
function createDb(params, cb){
  ...
  var dbConfig = getAppDbConf(name, fhconfig);
