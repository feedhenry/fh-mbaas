...
function removeDbMiddleware(req, res, next) {
  ...
  if (!_.isObject(model)) {
    ...
    // If its not an openshift3 app & the app was never migrated,
    // the per-app database will never have existed.
    if (!model.migrated && model.type !== 'openshift3') {
...
function removeModel(req, res, next) {
  req.appMbaasModel.remove(function(err, removed) {
    if (err) {
      return next(new Error('Removing app mbaas instance ', err));
    } else {
