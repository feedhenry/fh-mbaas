...
function getAppDbConf(db_name, config) {
...
/**
 * Construct the name of the app db. It should be in the format of:{domain}-{guid}-{env}.
 * It is the format being used by 3.x apps. We want to continue using it in both 3.x and 4.x for 2 reasons:
 * 1. Make the migration easier. Otherwise we have to figure out how to map the app db names between 3.x and 4.x
 * 2. Fix a bug in 4.x where if 2 environments have the same first 4 characters, the app will end up with using the same app db for 2 different environments.
 * @param {*} appMbaasModel 
 */
function buildAppDbName(appMbaasModel) {
  var appName = appMbaasModel.name;
  var expectedDbNamePattern = /.*?\-.{24}-/g; //{domain}-{guid}-{env}
  if (appName.match(expectedDbNamePattern)) {
    //continue using the existing appName for all the 3.x apps
    return appName;
  } else {
    //build app db name use the same pattern for 4.x apps
    //It should be the same as `getAppName` in https://github.com/fheng/fh-supercore/blob/master/lib/handlers/mbaas/mbaashelper.js#L60
    var guid = appMbaasModel.guid;
    var domain = appMbaasModel.domain;
    var env = appMbaasModel.environment;
    var app = guid.replace(/_|-/g, '0').toLowerCase();
    return `${domain}-${app}-${env}`;
  }
}
...
function createDbForAppTypes(types) {
  return function middleware(req, res, next) {
    ...
    var dbName = buildAppDbName(req.appMbaasModel);
    var dbConfig = getAppDbConf(dbName, config);
    try {
