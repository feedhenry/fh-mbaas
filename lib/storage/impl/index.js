...
var os = require("os");
var FileModel = require("../model/fileResource").FileResource;
...
root.registerFile = function(fileLocation, callback) {
  readFileSize(fileLocation, function(err, fileSize) {
    ...
    model.fakeSave(function(err, newModel) {
      if (err) {
...
root.generateURL = function(fileId, expiresIn, callback) {
  ...
  FileModel.fakeFindById(fileId, function(err, found) {
    if (err) {
...
root.getFileDetails = function(fildId, callback) {
  FileModel.fakeFindById(fildId, function(err, found) {
    if (err || !found) {
