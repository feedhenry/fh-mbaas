 ...
 * @param callback
 */
storage.registerFileForUpload = function(futureFileLocation, callback) {
  var data = {
    directory: path.dirname(futureFileLocation),
    fileName: path.basename(futureFileLocation),
    host: os.hostname(),

    // Must be updated when upload is finished
    size: 0
  };
  var model = new FileModel(data);
  model.save(function(err, newModel) {
...
/**
 * Update the file of a size once it's upload has finished.
 *
 * @param fileId The file ID
 * @param newSize The total size of the file
 * @param callback
 */
storage.updateFileSize = function(fileId, newSize, callback) {
  FileModel.findById(fileId, function(err, found) {
    if (err || !found) {
      logger.warn("Cannot obtain file model " + fileId);
      if (err) {
