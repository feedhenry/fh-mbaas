...
module.exports = function getEnvs(mbaas, appMbaas, fhconfig){
  ...
  appEnvs.FH_REDIS_HOST = fhconfig.value('redis.host');
  appEnvs.FH_REDIS_PORT = fhconfig.int('redis.port');
  if(fhconfig.value('redis.password') && fhconfig.value('redis.password') !== ''){
    appEnvs.FH_REDIS_PASSWORD = fhconfig.value('redis.password');
  }
