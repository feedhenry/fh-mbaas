...
function getSecondaryReplSetHost(cb) {
  var command = [
    'mongo --quiet --eval',
    ' "this.rs.status().members && this.rs.status().members.filter(function (x) { return x.stateStr === \"SECONDARY\" })[0].name"'
  ].join('');
...
function mongoExportWrapper(emitter, context, collectionName, index, total, cb) {
  ...
  getSecondaryReplSetHost(function (err, host) {
    if (err) {
    ...
    var dumpHost = host || uriObject.hosts[0].host;
    logger.info('Using host ' + dumpHost + ' to run mongodump');
