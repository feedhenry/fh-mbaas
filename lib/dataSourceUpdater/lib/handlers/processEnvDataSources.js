...
function formatDbUri(dbConf) {
  ...
  if(_.isArray(dbConf.host)){
    _.each(dbConf.host, function(host, index){

      if(_.isArray(dbConf.port)){
        config.hosts.push({
          host: host,
          port: dbConf.port[index] || 27017
        });
      } else {
        config.hosts.push({
          host: host,
...
function processEnvDataSources(params, callback) {
  ...
  logger.debug("processEnvDataSources", params);
  ...
  if (!dbConf) {
    logger.error("No Db Config For Environment ", envConfigEntry);
    //TODO: Error Handling
  ...
  logger.debug("processEnvDataSources", envConfigEntry);

  async.waterfall([
    function getEnvDataSourcesAndServices(envDataCb) {

      logger.debug("getEnvDataSourcesAndServices", envConfigEntry);

      async.parallel({
        dataSources: function listDataSources(dsCb) {
          logger.debug("listDataSources", {
        ...
        deployedServices: function listEnvDeployedServices(servCb) {
          logger.debug("listEnvDeployedServices", {
        ...
        if (err) {
          logger.error("Error Getting Data Sources And Services ", {error: err, params: params});
        } else {
          logger.debug("getEnvDataSourcesAndServices", dataSourcesAndServices);
        }
    ...
    function updateAllEnvDataSources(envDataSourcesAndServices, updateCb) {
      ...
      logger.debug("updateAllEnvDataSources", envDataSourcesAndServices);

      async.each(envDataSourcesAndServices.dataSources, function (dataSource, cb) {

        logger.debug("updateAllEnvDataSources", dataSource);
        var error;
        ...
        if (!deployedService) {
          ...
          error = {
            ...
            systemDetail: "No Service With guid " + serviceGuid + " is deployed to environment " + envConfigEntry.environment,
        ...
        updateSingleDataSource({
          ...
          if (err) {
            logger.error("Error Updating A Data Source ", err);
          }
          return cb();
        });
